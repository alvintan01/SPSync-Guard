...
/**
 * Servlet implementation class getBlacklist
 */
@WebServlet("/getRansomProcess")
public class getRansomProcess extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public getRansomProcess() {
        super();
		...
		if(MemberUtility.authenticate(sessionkey,auth)){
			try{			
				...
				PreparedStatement pstmt = conn.prepareStatement(
					"select * from t_ransomprocess where c_timestamp > '"+timestamp+"'");
				ResultSet rs=pstmt.executeQuery();
				PrintWriter out = response.getWriter();
				if(rs.next()){
					PreparedStatement pstmt2 = conn.prepareStatement(
							"select * from t_ransomprocess");
